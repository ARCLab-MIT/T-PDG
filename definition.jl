...
"""
    create_reduced_problem(rocket, tf)
...
"""
function create_reduced_problem(rocket::Rocket, tf::RealValue; environment = nothing)::PyObject

    # Dimensions
    H, n, m = Int(floor(tf/rocket.Δt))+1+Int(tf%rocket.Δt!=0), 7, 4

    # Variables
    X = cvxpy.Variable((n, H+1), name="X")
    U = cvxpy.Variable((m, H), name="U")

    # Parameters
    A = cvxpy.Parameter((n,n), name="A")
    B = cvxpy.Parameter((n,m), name="B")
    p = cvxpy.Parameter((n,H), name="p")
    x_init = cvxpy.Parameter(n, name="x_init")
    x_final = cvxpy.Parameter(n-1, name="x_final")
    dt = cvxpy.Parameter(name="dt")
    z_init = cvxpy.Parameter(H+1, name="z_init")
    # Rocket parameters
    m_wet = cvxpy.Parameter(name="m_wet")
    α = cvxpy.Parameter(name="α")
    ρ_min = cvxpy.Parameter(name="ρ_min")
    ρ_max = cvxpy.Parameter(name="ρ_max")
    

    # Objective
    objective = cvxpy.Minimize(dt*sum(U[:3]))

    # ODE Constraint
    problem_constraints = [py"""$X[:,1:] == $A@$X[:,:$H]+$B@$U+$p"""]

    # Unpack constraint list
    ...
    if constraints["Mass Bound"]
        append!(problem_constraints, z_init.<=X[-1])
        @constraint(mdl, [k=1:N], z[k]<=log(rocket.m_wet-rocket.α*rocket.ρ_min*t[k]))
        append!(problem_constraints, X[-1].<=cvxpy.reshape(cvxpy.log(m_wet*ones(1,H+1)'-α*ρ_min*(ones(1,H+1).*(1:1:H+1)')'),(H+1,)))
    end
    ...
    # Boundary conditions
    py"""$problem_constraints.append($X[:,0] == $x_init)"""
    py"""$problem_constraints.append($X[1:2,$H] == $x_final)"""
    ...
    problem = cvxpy.Problem(objective, problem_constraints)
    ...
    Δt = tf/(N-1)
    t = RealVector(0.0:Δt:tf+1)

    A.value, B.value, p_const = c2d(rocket.A_c, rocket.B_c, rocket.p_c, Δt) #noerr

    p.value = [p_const for i in 1:N]

    constraints = rocket.constraints

    x_init.value = vcat(rocket.r0, rocket.v0, [log(rocket.m_wet)])
    x_final.value = vcat(rocket.rf, rocket.vf)

    m_wet.value = rocket.m_wet
    α.value = rocket.α
    ρ_min.value = rocket.ρ_min
    ρ_max.value = rocket.ρ_max
    dt.value = tf/(N-1)
    z_init.value = log.(rocket.m_wet*ones(size(t)[1])-rocket.α*rocket.ρ_max*t)
    e_z_init = cvxpy.exp(-z_init)
    #μ_min.value = rocket.ρ_min*e_z_init
    #μ_max.value = rocket.ρ_max*e_z_init
    #δz = z-z_init
...
#function solve_upg(rocket::Rocket; environment = nothing)::Solution

    # State transition matrix 
 #   function Φ(Δt)
  #      return [cos(Δt)*ones(3,3) sin(Δt)*ones(3,3); -sin(Δt)*ones(3,3) cos(Δt)*ones(3,3)]
   # end

    # Rotation matrix
    #function Γ(t)
     #   return [sin(t)*ones(3,3) -cos(t)*ones(3,3); cos(t)*ones(3,3) sin(t)*ones(3,3)]
#    end

    # Thrust integral
 #   function i(t,t0,pv)
  #      return [integral(t0,t) 1pv]
   # end
    ...
    # State equation
    #function x(t,t0,x0)
     #   return Φ(t-t0)*x0 + Γ(t)*i(t,t0)

    # Define the nonlinear algebraic equations
 #   function f(rocket)
  #      return [rocket.r_f - rocket.r_0,
   #             rocket.v_f - rocket.v_0,
    #            rocket.p_r'*rocket.v]
    #end

    # Use root finding to find the values of x that make the function equal to zero
#    res = nlsolve(f, [0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5])
 #   x = res.zero
...
- `sol`: solution to the rocket landing problem for this `tf`.
"""
function solve_pdg_generalized(rocket::Rocket, tf::RealValue; environment = nothing, tight_constraints = nothing, initial_values = nothing)::Solution
